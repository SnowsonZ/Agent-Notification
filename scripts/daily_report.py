...
def token_text(value):
    ...
    if value < 1_000:
        return str(value)
    for factor, unit, decimals in ((1_000_000_000, 'B', 2), (1_000_000, 'M', 1), (1_000, 'k', 1)):
        if value >= factor:
...
class _Buckets:
    ...
    def point(self, provider, sid, stamp, *, input=0, cache=0, output=0, turn=False,
              title='', project='', fidelity='exact', state='unknown'):
...
def _zcode_data(home):
    ...
    now = time.time()
    for row in rows:
        sid, started, completed, input_tokens, cache_read = row[0], row[1], row[2], row[3], row[4]
        ...
        if tokenized:
            ...
            input_cls = max(int(input_tokens or 0) - int(cache_read or 0), 0) + int(cache_creation or 0)
...
def _codex_data(home, window_start):
    ...
    if index.exists():
        for line in index.open(errors='replace'):
            try:
        ...
        try:
            for line in path.open(errors='replace'):
                ...
                if kind == 'token_count':
                    ...
                    events.append((sid, stamp, input_cls,
        ...
        try:
            with path.open(errors='replace') as file:
                for line in file:
                    ...
                    if not isinstance(usage, dict) or message.get('role') != 'assistant':
                        continue
                    stamp = seconds(record.get('timestamp'))
                    if stamp <= 0 or stamp < window_start:
                        continue
                    input_cls = int(usage.get('input') or 0) + int(usage.get('cacheWrite') or 0)
                    output_cls = int(usage.get('output') or 0) + int(usage.get('reasoning') or 0)
                    results.append((row['session_id'], stamp, input_cls,
                                    int(usage.get('cacheRead') or 0), output_cls))
        except OSError:
            continue
    return results
...
def _kimi_data(home, window_start):
    ...
    for path in root.rglob('wire.jsonl'):
        try:
            if path.stat().st_mtime < window_start - 60:
                continue
            sid = next(part[len('session_'):] for part in path.parts if part.startswith('session_'))
        ...
        try:
            with path.open(errors='replace') as file:
                for line in file:
                    ...
                    if stamp <= 0 or stamp < window_start:
                        continue
                    input_cls = (int(usage.get('inputOther') or 0)
                                 + int(usage.get('inputCacheCreation') or 0))
                    output_cls = int(usage.get('output') or 0)
                    results.append((sid, stamp, input_cls,
                                    int(usage.get('inputCacheRead') or 0), output_cls))
        except OSError:
            continue
    return results
...
def scan_buckets(store, home, first_day, last_day, agent_stats=None):
    ...
    for sid, start, end, input_cls, cache_cls, output_cls, count_turn, activity in turns:
        ...
        buckets.spread('zcode', sid, start, end,
                       input=input_cls or 0, cache=cache_cls or 0, output=output_cls or 0,
    ...
    for day, tasks in buckets.tasks.items():
        ...
        for task in tasks.values():
            ...
            if total <= 0 and task['turns'] == 0 and task['fidelity'] != 'unavailable':
                continue
            records.append({
                'provider': task['provider'], 'session_id': task['session_id'],
                'title': task['title'] or f"{PROVIDER_NAMES.get(task['provider'], task['provider'])} · {task['session_id'][:12]}",
                'project': task['project'], 'first_at': round(task['first']),
...
def render_markdown(report):
    ...
    if not report['tasks']:
        ...
        lines += ['## 来源', '']
        for name, entry in sorted(totals['sources'].items(), key=lambda item: -item[1]['total_tokens']):
            lines.append(f"- {PROVIDER_NAMES.get(name, name)} 合计 {token_text(entry['total_tokens'])}"
                         f"（输入 {token_text(entry['input_tokens'])} · 缓存 {token_text(entry['cache_tokens'])}"
                         f" · 输出 {token_text(entry['output_tokens'])}）")
        lines += ['', '## 项目', '']
        for name, entry in sorted(totals['projects'].items(), key=lambda item: -item[1]['total_tokens']):
            lines.append(f"- {Path(name).name} 合计 {token_text(entry['total_tokens'])} tokens")
        lines += ['', '## 任务', '',
                  '| 任务 | 来源 | 项目 | 起止 | 输入 | 缓存 | 输出 | 合计 | 轮次 | 口径 |',
                  '|---|---|---|---|---|---|---|---|---|---|']
        for record in report['tasks']:
            span = (f"{clock_text(record['first_at'])}–{clock_text(record['last_at'])}"
                    if record['last_at'] > record['first_at'] else clock_text(record['first_at']))
            lines.append('| {title} | {provider} | {project} | {span} | {inp} | {cache} | {out} | {total} | {turns} | {fidelity} |'.format(
                title=record['title'].replace('|', '\\|'),
                provider=PROVIDER_NAMES.get(record['provider'], record['provider']),
                project=Path(record['project']).name if record['project'] else '—', span=span,
                inp=token_text(record['input_tokens']), cache=token_text(record['cache_tokens']),
...
def _write_report(root, report):
    ...
    payloads = ((directory / (report['date'] + '.md'), render_markdown(report)),
                (directory / (report['date'] + '.json'), json.dumps(report, ensure_ascii=False, indent=1) + '\n'))
...
def load_report(root, date_text):
    ...
    try:
        ...
        return None
    if isinstance(report, dict) and report.get('version') == REPORT_VERSION and report.get('date') == date_text:
        return report
...
def generate_overview(store, home, *, days=182, top=5):
    ...
    for offset in range(days):
        ...
        day_rows.append({key: int(totals.get(key) or 0) for key in CLASSES}
                        | {'date': text, 'total_tokens': int(totals.get('total_tokens') or 0),
    ...
    for offset in range(min(7, days)):
        ...
        report = live_today if day == today else load_report(store.root, day.isoformat())
        totals = (report or {}).get('totals', {})
        for target, merged in ((totals.get('sources') or {}, merged_sources),
    ...
    ranked = sorted(merged_projects.items(), key=lambda item: (-item[1]['total_tokens'], item[0]))[:max(1, top)]
    top_projects = [{'project': name, 'name': Path(name).name, **entry,
    ...
    today_row.update({'date': today.isoformat(), 'total_tokens': today_totals['total_tokens'],
                      'tasks': today_totals['tasks'], 'turns': today_totals['turns'],
                      'sources': sum(1 for entry in today_totals['sources'].values() if entry['total_tokens'] > 0),
                      'level': heat_level(today_totals['total_tokens'])})
    return {'generated_at': round(time.time()), 'days': day_rows, 'top_projects': top_projects,
            'week_sources': merged_sources, 'today': today_row}
