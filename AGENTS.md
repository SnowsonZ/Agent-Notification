...
- AppKit 前台状态更新需要服务主 run loop；不要用阻塞 sleep 轮询它。AXPress 返回成功不等于界面动作已完成，检查后置状态。
- 本轮结束不等于需求成功；运行状态和未读状态独立保存。打开失败不清未读；标记已处理必须使用 revision 校验，避免吞掉新事件。
...
- 工具拒绝某 App 时，不通过另一控制技术绕过；完成独立工作，准备可审阅探针，由用户或允许的环境验证。
