...
class InboxTests(unittest.TestCase):
    ...
    def event(self, event_id, time, attention=True, state='idle', token=None):
        return self.store.event('pi', 'session-a', event_id=event_id, timestamp=time,
                                state=state, attention=attention, token=token)
    ...
    def test_claude_setup_preserves_existing_hooks_and_is_idempotent(self):
        ...
        config.write_text(json.dumps({'model': 'existing', 'hooks': {'Stop': [{'hooks': [{'type': 'command', 'command': 'existing-command'}]}]}}))
        ...
        self.assertEqual(json.loads(content)['hooks']['Stop'][0]['hooks'][0]['command'], 'existing-command')
    ...
    def test_codex_initial_history_is_quiet_then_new_completion_is_unread(self):
        ...
        path.write_text(line('session_meta', {'id': 'fixture', 'originator': 'Codex Desktop'}) +
                        line('event_msg', {'type': 'task_complete', 'turn_id': 'old', 'last_agent_message': 'secret'}))
        collect_codex(self.store, home)
        self.assertFalse(self.store.rows()[0]['unread'])
        self.store.set_meta('started_at', 0)
        with path.open('a') as file:
            file.write(line('event_msg', {'type': 'task_complete', 'turn_id': 'new', 'last_agent_message': 'secret'}, '2026-02-01T00:00:00Z'))
        collect_codex(self.store, home)
        self.assertTrue(self.store.rows()[0]['unread'])
    ...
    def test_fork_history_does_not_attribute_parent_events_to_child(self):
        ...
        records = [
            {'type': 'session_meta', 'payload': {'id': 'parent', 'originator': 'Codex Desktop'}},
            {'type': 'event_msg', 'payload': {'type': 'task_complete', 'turn_id': 'parent-turn'}},
            {'type': 'session_meta', 'payload': {'id': 'child', 'originator': 'Codex Desktop'}},
        ...
        self.assertEqual([(e['session_id'], e['turn_id']) for e in events], [('child', 'child-turn')])
