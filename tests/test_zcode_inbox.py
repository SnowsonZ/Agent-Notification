...
class ZcodeInboxTests(unittest.TestCase):
    def setUp(self):
        ...
        with sqlite3.connect(self.index) as db:
            db.execute(
                "CREATE TABLE tasks (task_id,title,workspace_path,task_status,unread_at,updated_at,archived,deleted,last_unread_at)"
            )
            db.execute(
        ...
        with sqlite3.connect(self.runtime) as db:
            db.execute(
                "CREATE TABLE turn_usage (session_id,turn_id,status,started_at,completed_at)"
            )
            db.execute(
    ...
    def test_late_landed_turn_end_overcomes_earlier_stream_scan(self):
        # 评审 R2 两阶段拓扑：先扫到 part（旧实现以扫描墙钟记 event_at），结束行随后
        # 才落库且 completed_at 早于该墙钟——结束事件曾被判过期丢弃并烧毁 event_id，
        # 会话永久卡在 running。前置一个已落库回合：首次进 turn_usage 的 event_at
        # 归零迁移（zcode-turn-clock）只发生一次，不得当作对本缺陷的自愈。
        import time as _time

        now_ms = int(_time.time() * 1000)
        self.turn(
            "turn-base", status="completed", start=now_ms - 60000, end=now_ms - 50000
        )
        collect_zcode(self.store, self.home)
        self.assertEqual(self.store.rows()[0]["state"], "idle")
        self.part(updated=now_ms - 1000)
        collect_zcode(self.store, self.home)
        self.assertEqual(self.store.rows()[0]["state"], "running")
        self.turn("turn-live", start=now_ms - 2000, end=now_ms - 500)
        collect_zcode(self.store, self.home)
        self.assertEqual(self.store.rows()[0]["state"], "idle")
