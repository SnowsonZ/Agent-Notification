...
class OpenCodeManagedTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        register(
            self.store.root, "pane-1", "opencode", self.run_id, "/dev/ttys999", 4242
        )
        self.demo = self.root / "work" / "demo"
        self.demo.mkdir(parents=True, exist_ok=True)
        self.env = {
            "SESSION_MANAGER_RUN_ID": self.run_id,
    ...
    def event(self, name, sid="ses_a", **extra):
        record_event(
            "opencode",
            {
                ...
                "cwd": extra.pop("cwd", str(self.demo)),
                **extra,
            },
            env=self.env,
        )
    ...
    def test_plugin_events_drive_full_lifecycle(self):
        ...
        self.assertEqual(
            row["locator"],
            {"kind": "managed", "run_id": self.run_id, "session_id": "ses_a"},
        )
    ...
    def test_open_uses_binding_probe(self):
        ...
        with patch("inbox.subprocess.run") as run:
            run.return_value = subprocess.CompletedProcess(
                [], 0, json.dumps({"activation_call_completed": True}), ""
            )
    ...
    def test_open_resumes_when_binding_dead(self):
        ...
        self.assertTrue(
            any("managed_resume" in str(call) for call in output.write.call_args_list)
        )

    def test_open_refuses_resume_when_binding_still_alive(self):
        # 评审 R3：probe 非零 ≠ 绑定死亡——setUp 里的原绑定运行锁仍被持有（会话
        # 进程活着），validate 因前台组校验失败拒绝时，旧实现照样 launch 复制出
        # 第二个会话并把未读自动确认。正确行为：拒绝恢复、保留未读、报出原因。
        self.event("SessionStart")
        self.event("Stop")
        row = self.store.rows(unread_only=True)[0]
        with (
            ...
            run.return_value = subprocess.CompletedProcess(
                [], 1, "", "foreground terminal ownership could not be verified"
            )
            code = open_session(self.store, row["id"], row["revision"])
        self.assertNotEqual(code, 0)
        launch.assert_not_called()
        self.assertTrue(self.store.get(row["id"])["unread"])
        self.assertTrue(
            any(
                "refusing to resume" in str(call)
                for call in errors.write.call_args_list
            )
        )
    ...
    def test_open_prefers_live_rebound_binding(self):
        ...
        with (
            patch("inbox._live_binding_for", return_value=new_run) as lookup,
            patch("inbox.subprocess.run") as run,
        ):
            run.side_effect = [
                subprocess.CompletedProcess([], 1, "", "refused"),
    ...
    def test_live_binding_lookup_matches_session_and_excludes_dead(self):
        ...
        with patch("session_binding.alive", return_value=True):
            self.assertEqual(
                _live_binding_for(
                    self.store.root, "opencode", "ses_a", exclude=self.run_id
                ),
                None,
            )
            new_run = "9" * 32
            ...
            self.addCleanup(lease.close)
            register(
                self.store.root, "pane-9", "opencode", new_run, "/dev/ttys996", 4243
            )
            with patch("session_binding.alive", return_value=True):
                ...
                with connect(self.store.root) as db:
                    db.execute(
                        "UPDATE bindings SET session_id=? WHERE run_id=?",
                        ("ses_a", new_run),
                    )
                self.assertEqual(
                    _live_binding_for(
                ...
                with self.assertRaises(ValueError):
                    _live_binding_for(
                        self.store.root, "opencode", "ses_a"
                    )  # exclude 未传 → 两个都算 → 歧义
    ...
    def test_open_resumes_pi_and_kimi_with_session_flag(self):
        ...
        lifecycle = {
            "pi": ("session_start", "agent_settled"),
            "kimi": ("SessionStart", "Stop"),
        }
        for provider, run_id, sid in (
            ("pi", "1" * 32, "pi-sess-uuid"),
            ...
            record_event(
                provider,
                {"event": start_event, "session_id": sid, "cwd": str(self.demo)},
                env=provider_env,
            )
            record_event(
                provider, {"event": end_event, "session_id": sid}, env=provider_env
            )
            row = self.store.rows()[0]
...
class OpenCodePluginTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.env = {
            "SESSION_MANAGER_RUN_ID": "b" * 32,
            "SESSION_MANAGER_PYTHON": "node",
            "SESSION_MANAGER_BINDING_SCRIPT": str(reporter),
            "OUT": str(out),
        }
    ...
    def test_sdk_events_map_to_inbox_names(self):
        reports = self.feed(
            ...
            {"type": "session.idle", "properties": {"sessionID": "ses_1"}},
            {
                ...
                "properties": {"type": "ask", "sessionID": "ses_1"},
            },
            {
                "type": "permission.updated",
                "properties": {"type": "allow", "sessionID": "ses_1"},
            },
        )
        self.assertEqual(
            [(r["event"], r["session_id"]) for r in reports],
            [
                ("SessionStart", "ses_1"),
                ("UserPromptSubmit", "ses_1"),
                ("Stop", "ses_1"),
