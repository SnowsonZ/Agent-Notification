...
状态：当前设计方案，待接入验证，尚未转为可执行规格。前期工具比较见 [市场调研](../research/2026-09-13-market-survey.md)；文档管理约定见 [项目入口](../../README.md)。

当前交付已推进到 [统一收件箱 v0.1](../specs/unified-inbox.md)：原生 App、事件队列和来源采集已实现并完成列表 UI 验证。下文保留原方案背景，实际能力与限制以该交付规格为准。
