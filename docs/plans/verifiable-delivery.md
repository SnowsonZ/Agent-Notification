...
| H0925-6 | Zcode「前往会话」两份规格描述矛盾 | 已决定（2026-09-26）：现役只停在搜索结果页；自动导航规格标为非现役 |
...
**2026-09-26 起，未关闭事项统一在 [待办清单](backlog.md) 跟踪**，本节只保留历史：其中仍未完成的 T2、V6、E2（R6、X5）、E6、E7 已迁到待办清单 B1、B2、B6–B9。
...
| D3 | 执行方是否使用单独的 GitHub 身份（机器账号或 GitHub App） | 决定是否能接上 L4 自动合并（E1） | 已改选（2026-09-26）：Agent 用单独账号 `Snowson`，R0/R1 由专用 App 批准（§10）；仓库侧改动已合并（PR #31）；用户侧设置按规范 §5 第 8、9 步于 2026-09-26 完成（ruleset 已重新导入、environment `auto-merge` 只允许 main、App 已安装）；实测完成：R0 的 PR #32 由 App 批准后自动合并（App 需 Contents 写权限，H0926-6），R2 以上须用户批准（规范 §6） |
...
| # | 事项 | 负责人 | 前置 | 状态 |
|---|---|---|---|---|
| T1 | 执行 [trial-001](trial-001-dr14.md)（补 DR14 测试），走完整流程 | 用户指定的执行方 | A2–A5 | 完成（2026-09-26，PR #11；执行方 OpenCode） |
| T2 | 填写 trial-001 的试跑记录，与 v0.8.0 基线对比；两周后补逃逸缺陷 | 用户 + 评审方 | T1 | 已填（本次数据见 trial-001「试跑记录」）；逃逸缺陷观察至 2026-10-10 |
| T3 | 根据试跑暴露的问题调整规则、模板或检查（R3） | 设计评审方 | T2 | 完成（2026-09-26）：试跑暴露的 H0926-2（执行方守卫与缺口清单冲突）、H0926-4（base_tests 误拦补测试的 PR）已修复并补回放用例；回放用例改由评审方加入（任务模板）。之后每次试跑暴露的问题按缺陷编号处理，不再单列 |
...
| V2 | Codex PreToolUse 钩子（载荷格式、信任门禁） | 按 harness 规范 §5 第 4 步配置后同上 | 完成（2026-09-26）：Codex 支持项目级 `.codex/hooks.json`，信任门禁与载荷格式在临时仓库实测；本仓库改用项目级钩子（PR #37），用户信任后在仓库内实测：普通命令放行，`gh pr merge` 被守卫拦截（规范 §6） |
...
| V7 | Zcode 工作区钩子在真实 Zcode 中拦截 | 用户按规范 §5 第 6 步信任后，让 Zcode 执行设置覆盖变量的命令与编辑 `harness/` 文件，应被拒绝 | 完成（2026-09-26）：桌面版两项探针均被拒绝、文件未改；CLI 与 TUI 不执行工作区钩子且无参数可开（源码核查），用 Zcode 当执行方时用桌面版 |
| V4 | 写入器重构在真机上行为不变（W6 隐藏标题保留项目名；R8 进行中计数） | 用户真机抽查组件 | 完成（2026-09-26）：用户在真机确认 W6（隐藏任务标题时项目名照常显示）与 R8（组件「进行中」与收件箱一致）。验收中发现最近任务的今日 token 从未显示（H0926-7 会话 ID 解码、H0926-8 签名不含用量），经[任务 004](task-004-today-tokens.md)（PR #39）修复；重新构建后快照 8 条中 6 条带今日 token（修复前 0 条），用户确认组件显示 |
| V5 | 每周 quality workflow 首次运行 | 合并后在 Actions 中手动触发一次 | 完成（2026-09-26 手动触发 run 36220317479 通过） |
