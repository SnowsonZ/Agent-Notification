...
| V7 | Zcode 守卫在真实 Zcode 中拦截 | 用户运行 `python3 harness/zcode_hook.py install` 后，让 Zcode（CLI 与桌面版各一次）执行设置覆盖变量的命令与编辑 `harness/` 文件，应被拒绝 | 项目级方案实测失败已弃用（CLI 不执行工作区钩子）；改为用户级钩子，待用户安装后实测 |
