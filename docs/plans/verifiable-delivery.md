...
| D3 | 执行方是否使用单独的 GitHub 身份（机器账号或 GitHub App） | 决定是否能接上 L4 自动合并（E1） | 已改选（2026-09-26）：Agent 用单独账号 `Snowson`，R0/R1 由专用 App 批准（§10）；仓库侧改动已合并（PR #31）；用户侧设置按规范 §5 第 8、9 步于 2026-09-26 完成（ruleset 已重新导入、environment `auto-merge` 只允许 main、App 已安装）；R0 实测即本条改动所在的 PR |
