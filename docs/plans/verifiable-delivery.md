...
| V2 | Codex PreToolUse 钩子（载荷格式、信任门禁） | 按 harness 规范 §5 第 4 步配置后同上 | 部分完成（2026-09-26）：Codex 支持项目级 `.codex/hooks.json`，信任门禁与载荷格式已在临时仓库实测（规范 §6）；本仓库改用项目级钩子，待用户信任后实测守卫拦截 |
