...
| V4 | 写入器重构在真机上行为不变（W6 隐藏标题保留项目名；R8 进行中计数） | 用户真机抽查组件 | 进行中（2026-09-26）：新版构建安装后发现最近任务的今日 token 从未显示（H0926-7 会话 ID 解码、H0926-8 签名不含用量），交[任务 004](task-004-today-tokens.md)；W6、R8 待用户在真机确认 |
