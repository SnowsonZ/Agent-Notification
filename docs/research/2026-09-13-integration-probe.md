...
新增 [Claude ID 解析器](../../scripts/claude_locator.py)，按明确 ID 读取 Desktop 元数据；跨账号多匹配、损坏元数据、归档、缺失记录均不返回可用链接。9 项测试通过，其中 5 项验证定位、4 项验证接收器数据最小化和不影响父 hook 决策。测试入口见 [README](../../README.zh-CN.md)。
