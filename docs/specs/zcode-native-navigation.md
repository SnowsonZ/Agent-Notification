...
状态：**非现役**（2026-09-26 用户确认，H0925-6）。现役的 Zcode「前往会话」只打开任务搜索、预填标题并停在结果页，不自动打开任务、不做任务 ID 后置核验，合同见[统一收件箱](unified-inbox.md) IN14；Zcode CLI 会话未接入。本文保留自动导航适配器的设计与 2026-09-14 真实运行证据（首次真实 AX 自动导航及任务 ID 后置核验曾通过，本地日志 exit_code=0），作为重新启用时的依据；其中的核验规则仍约束现存的共用代码（索引读取、路径比对）。
...
**口径（H0925-6，2026-09-26 用户确认）**：下表 ZN1–ZN5 约束仍在代码中的索引读取、路径比对、原生边界与自检，照常由测试覆盖；ZN6、ZN7 属于自动导航流程，该流程当前不在任何入口启用，不作为现役验收。
