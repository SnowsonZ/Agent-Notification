...
- **日报窗口顶部**新增两个分段控件：`日 | 周 | 月` 和 `USD | CNY`，选择保存在 `UserDefaults`，币种默认 CNY。**日**即现有的总览与当日详情。
- **周、月视图**，图表一律用 Swift Charts：
  1. 汇总卡：金额大字、三类 token、任务数、环比（▲▼ 百分比；上期为 0 时不显示）、未定价提示「另有 X tokens 未定价」。
  2. 逐天堆叠柱图，可在「金额 / token」两种度量间切换。月视图叠加一条本月累计金额折线，与柱图共用 x 轴。点柱子进入当日详情。
  3. 占比区：harness / model 两个横向条形榜，各取 Top 6，其余合并为「其他」。
  4. Top 5 项目。
  5. 周期导航：‹ 上一期 · 本期 · 下一期 ›，下一期不能超过本期。
- **日视图补充**：
  - 汇总卡、来源行、项目行、任务卡都附金额。
  - 新增 model 占比条。
  - 总览热力图增加「按金额」着色开关。金额分级阈值取近 182 天有消耗日的日金额（CNY 视图）的 50 / 75 / 90 分位，写成常量，与 token 阈值的做法相同。
- **悬浮提示**沿用现有自定义卡片，新增金额行：`金额：¥12.34（输入 ¥a · 缓存 ¥b · 输出 ¥c）`，有未定价时追加一行 `未定价：X tokens`。
- **注脚**写明：按标价估算、汇率及其日期、价格表更新时间、首版不含阶梯价。
...
| U10 | 界面 | 真实数据下周、月、币种切换截图核对，悬浮卡数字与 `inbox usage` 一致 |
