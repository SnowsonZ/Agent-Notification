...
状态：现役 v0.7.3（2026-09-21）。本文按时间追加演进记录，最新能力与验收以最近章节为准；早期里程碑（如 v0.4.0 已构建并通过完整 bundle 签名校验、重做界面：列表懒加载、品牌主色、苹果风格排版；Zcode 辅助功能授权已生效，"打开会话"停在任务搜索结果页的语义已实测；CLI 检测与进程 PATH 解耦）保留作历史证据。统计类能力（工作日报、热力图与 Top 项目）独立成篇：[daily-report.md](daily-report.md)。
