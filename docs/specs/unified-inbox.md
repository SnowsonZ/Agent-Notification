...
状态：v0.2.1 已构建并通过完整 bundle 签名校验；新增 Pi 标题、应用图标、分页、成功打开后自动处理和系统通知。列表翻页与通知提交已实测；Zcode 辅助功能授权已生效，模拟事项经 App「打开会话」完成一次真实导航并按点击时 revision 自动确认，完整 GUI 闭环已验收（2026-09-14，证据见文末）。
...
- 系统设置与 App 均显示通知开启，模拟事件的通知请求被系统接受（App 的通知去重记录含 token）。点击通知的行为改为仅置前收件箱；原先的自动打开实现（pendingNotificationOpen）已移除。
