...
状态：v0.2 已构建并通过完整 bundle 签名校验；新增 Pi 标题、应用图标、分页、成功打开后自动处理和系统通知。列表翻页与通知提交已实测；新版 App 的 Zcode 辅助功能授权尚未验证生效，成功导航后的自动处理有单元覆盖，完整 GUI 闭环待补。
...
App 运行时通知新的待处理事件；首次快照静默，不补发历史堆积。铃铛按钮控制通知，系统需允许通知。通知按事项和事件 token 去重，点击通知走相同的打开与 revision 校验。通知内容只包含来源、状态和展示标题。
...
- 系统设置与 App 均显示通知开启，模拟事件的通知请求被系统接受；未独立验收点击系统通知后的导航。
- 模拟事项打开 Zcode 被辅助功能检查拒绝后仍为未读。用户已授权更新权限；重新切换开关及添加当前应用后仍未通过，随后 Computer Use 连接中断。后续需恢复界面连接、核对旧授权记录并重新添加，再验收成功自动处理。仅选择其他行未改变其他应用授权。
