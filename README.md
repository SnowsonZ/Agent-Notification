...
已有可运行的原生应用“会话通知”：待处理列表、按最新时间排序的懒加载列表、Pi 会话名、独立图标、成功打开自动处理、系统通知及菜单栏计数；启动时检测本机 CLI，在「新建会话」行提供 Claude/Codex/Pi/Kimi 的一键目录启动（iTerm2 新标签）。用 `bin/session-manager app` 打开。完整使用与验收边界见 [统一收件箱](docs/specs/unified-inbox.md)。
