...
- That is the developer bundle: the app runs the repository's scripts and venv in place. The Release dmg is built with `python3 scripts/build_inbox_app.py --standalone`, which copies the scripts, `bin/session-manager`, `zcode-focus` and the pure-Python deps (`requirements-standalone.txt`) into `Contents/Resources`, so it runs without a checkout; it needs a python3 3.11+ on the machine (Homebrew works; macOS's bundled 3.9 does not — the minimum was raised to 3.11 on 2026-09-21). Both bundles share one codebase and switch on whether `Resources/pylib` exists.
- `scratch/` and `build/` are not tracked by git; the steps above create them. The virtualenv is a runtime dependency of the developer-bundle app, not a disposable artifact: when cleaning `scratch/` (including `git clean`) keep `scratch/iterm-probe-venv`. If it gets deleted the inbox shows “The file ‘python’ doesn’t exist.” and the CLI reports “Missing runtime”; re-running the two venv commands above restores it (data lives in `~/.local/state/session-manager` and is unaffected).
...
The complete documentation (specifications, design plans, dated research records, and reviews) is available in Chinese under [`docs/`](docs/). Start with [the unified inbox specification](docs/specs/unified-inbox.md); the latest cross-project health review is the [2026-09-21 project assessment](docs/review/2026-09-21-project-assessment.md). For the verifiable-delivery harness, start with [harness/README.md](harness/README.md) (vision, sources, and how each design principle maps to a component). The underlying theory is the research report [Low-intervention, continuous, high-quality delivery with AI agents](docs/research/2026-09-23-agent-delivery-theory.md); the plan and decision log are in [docs/plans/verifiable-delivery.md](docs/plans/verifiable-delivery.md), the baseline in the [2026-09-25 harness baseline review](docs/review/2026-09-25-harness-baseline.md), and open items in the [backlog](docs/plans/backlog.md).
